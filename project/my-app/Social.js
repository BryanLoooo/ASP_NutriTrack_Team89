...
const Social = ({ navigation }) => {
  ...
  const uploadImage = async (imageUri) => {
    ...
    try {
      ...
      const storageRef = ref(
        storage,
        `imagesPosted/${Date.now()}-${currentUserId}.jpg`
      );
  ...
  const handleLike = async (id, currentLikes, likedBy) => {
    ...
    if (newLikedBy.includes(currentUserId)) {
      ...
      console.log(
        "Heart icon clicked when its already liked, user does not want to like the image"
      );
    }
        ...
        {!addingPost && (
          <View
            style={[
              ...
              posts.length > 0 ? styles.headerWelcome : styles.middleWelcome,
            ]}
          >
            <Text
              style={[styles.welcomeText, { color: currentTheme.textColor }]}
            >
              Welcome to NutriTrack's social page, a space where you can share
              and explore the health journeys of fellow community members!
            </Text>
          </View>
        )}
        ...
        <View style={styles.mainContentContainer}>
          {loading ? (
            ...
            <>
                ...
                renderItem={({ item }) => (
                  <View key={item.id} style={styles.postContainer}>
                    ...
                    <View style={styles.likeContainer}>
                      <TouchableOpacity
                        onPress={() =>
                          handleLike(item.id, item.likes, item.likedBy)
                        }
                      >
                        <FontAwesome
              ...
              <TouchableOpacity
                style={styles.fixedAddPostButton}
                onPress={() => {
